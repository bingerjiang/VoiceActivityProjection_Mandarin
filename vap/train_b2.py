...
class VAPModel(VapGPT, pl.LightningModule):
    ...
    def metrics_step(self, preds, targets, split="val"):
        ...
        if preds["pred_backchannel"] is not None and preds["pred_backchannel"].numel()!=0:
            m["f1"]["bp"].update(
    ...
    def shared_step(
        ...
        if labels.shape != out["logits"].shape:
           # pdb.set_trace()
